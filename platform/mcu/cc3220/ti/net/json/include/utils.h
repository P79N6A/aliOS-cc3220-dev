...
/*---------------------------------------------------------------------------*/
    ...
    #define MemMove      memmove
        ...
        void    UlToA       (_I_  uint32_t    val , __O  char *   buf) ;    /* No native _ultoa in __TI_ARM__ */
    #elif defined ewarm
        #include <string.h>
        #define StrNiCmp     strncasecmp
        void    LToA (_I_  int32_t     val  ,__O  char *    buf   ) ; /* No native _ltoa in ewarm */
        void    UlToA       (_I_  uint32_t    val , __O  char *   buf) ;    /* No native _ultoa in ewarm */
	#elif defined __GCC__
		#include <strings.h>
		#define StrNiCmp     strncasecmp
		void    LToA (_I_  int32_t     val  ,__O  char *    buf   ) ; /* No native _ltoa in gcc */
		void    UlToA       (_I_  uint32_t    val , __O  char *   buf) ;    /* No native _ultoa in gcc */
    #else
    ...
    void    MemSet      (__O  void *    out_buf   , _I_  int      value   , _I_ size_t   size) ;
    void    MemCpy      (__O  void *    out_buf   , _I_  void *   in_buf  , _I_ size_t   size) ;
    void    MemMove     (__O  void *    out_buf   , _I_  void *   in_buf  , _I_ size_t   size) ;
    int     StrNiCmp    (_I_  char *    string1   , _I_  char *   string2 , _I_ size_t   size) ;
    void    LToA        (_I_  int32_t     val       , __O  char *   buf                        ) ;
    void    UlToA       (_I_  uint32_t    val       , __O  char *   buf                        ) ;
    /*-----------------------------------------------------------------------*/
#endif /* USE__STANDRAD_LIBS */
/*---------------------------------------------------------------------------*/
...
json_rc_T   EnsureArrayAccomodatesIndex (_IO_ void *                           json_internal   ,
                                         ...
                                         _I_  bool                          full_parse_mode  ) ;
/*---------------------------------------------------------------------------*/
void   UpdateBestCaseRc (_IO_ json_rc_T *  best_case_rc  ,_I_  json_rc_T    new_rc);
